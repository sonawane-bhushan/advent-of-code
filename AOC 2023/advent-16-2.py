...
for x in range(len(input_array)):
    startingPoints.append((x, 0, 'R'))
    startingPoints.append((x, len(input_array[0]) - 1, 'L'))

for y in range(len(input_array[0])):
    startingPoints.append((0, y, 'D'))
    startingPoints.append((len(input_array) - 1, y, 'U'))

print(max(count(point) for point in startingPoints))